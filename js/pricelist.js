...
function showAllItems(){
    $("#search_results").load('pricelist/showAllItems', function(){
       $("#all_table").DataTable({paging:false});
       $("th, td").addClass('text-center').css('text-transform', 'uppercase');
...
function editSku(skuId, skuName, skuCount, skuDesc) {
    ...
    $('#main_modal .modal-body').load('pricelist/editSku',
            {'skuId': skuId, 'skuName': skuName, 'skuCount': skuCount, 'skuDesc': skuDesc}, function () {
...
function editCategory(categoryId, categoryName, categoryDesc) {
    ...
    $('#main_modal .modal-body').load('pricelist/editCategory',
            {'categoryId': categoryId, 'categoryName': categoryName, 'categoryDesc': categoryDesc}, function () {
        $("label").addClass('label label-default');
    });
    $('#main_modal').modal('show');
}
function editSupplier(supplierId, supplierName, supplierAddress, supplierTelephone, supplierMobile, supplierEmail) {
    ...
    $('#main_modal .modal-body').load('pricelist/editSupplier',
            {'supplierId': supplierId, 'supplierName': supplierName, 'supplierAddress': supplierAddress, 'supplierTelephone': supplierTelephone, 'supplierMobile': supplierMobile, 'supplierEmail': supplierEmail}, function () {
        $("label").addClass('label label-default');
    });
    $('#main_modal').modal('show');
}
function editCustomer(customerId, customerName, customerAddress, customerTelephone, customerMobile, customerEmail) {
    ...
    $('#main_modal .modal-body').load('pricelist/editCustomer',
            {'customerId': customerId, 'customerName': customerName, 'customerAddress': customerAddress, 'customerTelephone': customerTelephone, 'customerMobile': customerMobile, 'customerEmail': customerEmail}, function () {
        $("label").addClass('label label-default');
    });
    $('#main_modal').modal('show');
}
function editItem(itemId, itemName, itemCategory, itemDesc) {
    ...
    $('#main_modal .modal-body').load('pricelist/editItem',
            {'itemId': itemId, 'itemName': itemName, 'itemCategory': itemCategory, 'itemDesc': itemDesc}, function () {
        $("label").addClass('label label-default');
    });
    $('#main_modal').modal('show');
}
function editCostNotes(costId, notes, cost, skuName, skuCount, itemId, itemName) {
    $('#main_modal .modal-title').html("Edit Cost Notes");
    $('#main_modal .modal-body').load('pricelist/editCostNotes',
            {'costId': costId, 'notes': notes, 'cost':cost, 'skuName':skuName, 'skuCount':skuCount, 'itemId': itemId, 'itemName': itemName}, function () {
        $("label").addClass('label label-default');
    });
    $('#main_modal').modal('show');
}
...
function updateCostNotes(costId, notes, cost, itemId, itemName) {
    if($.isNumeric(cost)){
        $.ajax({
            url: 'pricelist/updateCostNotes',
            type: 'post',
            dataType: 'text',
            data: {'costId': costId, 'cost':cost, 'notes': notes}
        }).done(function (data) {
            if (data) {
                $("#main_modal").modal('hide');
                showCostPrice(itemId, itemName);
                mainAlert("Successfully updated item cost for "+itemName+"!");
            }
            else {
                modalAlert("Failed updating item cost for "+itemName+"!");
            }
        });        
    }
    else{
